...
def login(username, password, pincode):
    ...
    cookie_raw = str(session.cookies)
    re_com = re.compile(r'Cookie (.*?) for')
    cookie = re.findall(re_com,cookie_raw)
    cookie = '; '.join(cookie)
    cookies = {}
    ...
    redirect_url = 'http://weibo.cn/5020817543/profile'
    #html = requests.get(redirect_url,cookies=cookies).text
    #print(html)
    # print(cn, com, mcn)
