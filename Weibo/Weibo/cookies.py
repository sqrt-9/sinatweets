...
import requests
import logging
from Weibo.accountDispose import account
...
def initCookie(rconn, spiderName):
    ...
    for weibo in weibo_account:
        if rconn.get("{0}:Cookies:{1}--{2}".format(spiderName, weibo['username'], weibo['password'])) is None:  #'weibo:Cookies:帐号--密码'，为None即为不存在。
    ...
    cookieNum = "".join(str(i) for i in rconn.keys()).count("weibo:Cookies")
